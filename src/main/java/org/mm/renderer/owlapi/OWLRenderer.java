...
public class OWLRenderer extends ReferenceRendererConfiguration
        implements Renderer, DeclarationRenderer, MappingMasterParserConstants {
    ...
    public static final int ReferenceValueTypes[] = { OWL_CLASS, OWL_NAMED_INDIVIDUAL, OWL_OBJECT_PROPERTY,
            OWL_DATA_PROPERTY, XSD_INTEGER, XSD_STRING, XSD_FLOAT, XSD_DOUBLE, XSD_SHORT, XSD_BOOLEAN, XSD_TIME,
            XSD_DATETIME, XSD_DURATION };
    ...
    public static final int DataPropertyValueTypes[] = { XSD_STRING, XSD_BYTE, XSD_SHORT, XSD_INTEGER, XSD_FLOAT,
            XSD_DOUBLE, XSD_BOOLEAN, XSD_TIME, XSD_DATETIME, XSD_DATE, XSD_DURATION };
    ...
    public OWLRenderer(OWLOntologySource ontologySource) {
        objectFactory = OWLObjectFactory.newInstance(ontologySource);
        referenceRenderer = new OWLReferenceRenderer(objectFactory);
    ...
    public Optional<OWLRendering> renderOWLClassDeclaration(OWLClassDeclarationNode declarationNode)
        ...
        Optional<OWLClassRendering> declaredClassRendering = entityRenderer
                .renderOWLClass(declarationNode.getOWLClassNode(), true);
        ...
        /*
         * By default, the class declaration node will produce one OWL class declaration
         * axiom
         */
        ...
        if (declarationNode.hasOWLEquivalentClassesNode()) {
            for (OWLEquivalentClassesNode equivalentClassesNode : declarationNode.getOWLEquivalentClassesNodes()) {
                for (OWLClassExpressionNode node : equivalentClassesNode.getClassExpressionNodes()) {
                    Optional<OWLClassExpressionRendering> classExpressionRendering = classExpressionRenderer
                            .renderOWLClassExpression(node);
                    if (classExpressionRendering.isPresent()) {
                        OWLClassExpression classExpression = classExpressionRendering.get().getOWLClassExpression();
                        OWLEquivalentClassesAxiom axiom = objectFactory.createOWLEquivalentClassesAxiom(declaredClass,
                                classExpression);
                        axioms.add(axiom);
                        axioms.addAll(classExpressionRendering.get().getOWLAxioms()); // add any existing axioms
                    }
        ...
        if (declarationNode.hasAnnotationFactNodes()) {
            for (AnnotationFactNode annotationFactNode : declarationNode.getAnnotationFactNodes()) {
                ...
                Optional<? extends OWLPropertyRendering> propertyRendering = entityRenderer
                        .renderOWLAnnotationProperty(node);
                ...
                if (propertyRendering.isPresent() && annotationValueRendering.isPresent()) {
                    ...
                    OWLAnnotationAssertionAxiom axiom = objectFactory.createOWLAnnotationAssertionAxiom(property,
                            classIri, annotationValue);
                    axioms.add(axiom);
                }
            }
        }
    ...
    public Optional<OWLRendering> renderOWLIndividualDeclaration(OWLIndividualDeclarationNode individualDeclarationNode)
        ...
        Optional<OWLNamedIndividualRendering> declaredIndividualRendering = entityRenderer
                .renderOWLNamedIndividual(individualNode, true);
        ...
        if (individualDeclarationNode.hasSameAs()) { // We have a SameAs clause
            List<OWLNamedIndividualNode> otherIndividualNodes = individualDeclarationNode.getOWLSameAsNode()
                    .getIndividualNodes();
        ...
        if (individualDeclarationNode.hasDifferentFrom()) { // We have a DifferentFrom clause
            List<OWLNamedIndividualNode> otherIndividualNodes = individualDeclarationNode.getOWLDifferentFromNode()
                    .getIndividualNodes();
    ...
    private Set<OWLAxiom> processFactsClause(OWLNamedIndividual individual, List<FactNode> factNodes)
        ...
        for (FactNode factNode : factNodes) {
            ...
            if (propertyRendering.isPresent()) {
                ...
                Optional<OWLPropertyAssertionRendering> propertyAssertionRendering = entityRenderer
                        .renderOWLPropertyAssertion(propertyAssertionNode);
                if (propertyAssertionRendering.isPresent()) {
                    OWLPropertyAssertionObject propertyAssertion = propertyAssertionRendering.get()
                    ...
                    if (property instanceof OWLObjectProperty) {
                        ...
                        OWLObjectPropertyAssertionAxiom axiom = objectFactory.createOWLObjectPropertyAssertionAxiom(op,
                                individual, value);
                        axioms.add(axiom);
                        ...
                        OWLDataPropertyAssertionAxiom axiom = objectFactory.createOWLDataPropertyAssertionAxiom(dp,
                                individual, value);
                        axioms.add(axiom);
                    }
                }
     ...
     */
    private Set<OWLAxiom> processAnnotationClause(OWLNamedIndividual individual,
     ...
     */
    private Set<OWLAxiom> processSameAsClause(OWLNamedIndividual individual,
            List<OWLNamedIndividualNode> otherIndividualNodes) throws RendererException {
        Set<OWLAxiom> axioms = new HashSet<>();
        for (OWLNamedIndividualNode individualNode : otherIndividualNodes) {
            Optional<OWLNamedIndividualRendering> otherIndividual = entityRenderer
                    .renderOWLNamedIndividual(individualNode, false);
     ...
     */
    private Set<OWLAxiom> processDifferentFromClause(OWLNamedIndividual individual,
            List<OWLNamedIndividualNode> otherIndividualNodes) throws RendererException {
        Set<OWLAxiom> axioms = new HashSet<>();
        for (OWLNamedIndividualNode individualNode : otherIndividualNodes) {
            Optional<OWLNamedIndividualRendering> otherIndividual = entityRenderer
            ...
            if (otherIndividual.isPresent()) {
                ...
                OWLDifferentIndividualsAxiom axiom = objectFactory.createOWLDifferentIndividualsAxiom(individual,
                        differentIndividual);
                axioms.add(axiom);
            }
        }
