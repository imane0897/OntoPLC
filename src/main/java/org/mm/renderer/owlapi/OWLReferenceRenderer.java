...
public class OWLReferenceRenderer implements ReferenceRenderer, MappingMasterParserConstants {
    ...
    public OWLReferenceRenderer(OWLObjectFactory objectFactory) {
    ...
    public Optional<OWLReferenceRendering> renderReference(ReferenceNode referenceNode) throws RendererException {
        ...
        if (sourceSpecificationNode.hasLiteral()) {
            ...
            if (referenceNode.hasLiteralType()) {
                ...
                /*
                 * If the source specification node is written as a literal value without
                 * datatype, e.g., @"XYZ", then MM will assume it is an OWL entity. The
                 * assignments below assume that the entity name and its label are equal to the
                 * literal value.
                 */
                ...
                /*
                 * Create the OWL entity based on the inputs of its name, label and language
                 * tag. The method createOWLEntity will check first if the name (or label)
                 * exists in the ontology.
                 */
                OWLEntityReferenceRendering entityRendering = null;
                Optional<OWLEntity> createdEntity = createOWLEntity(entityName, entityLabel, languageTag,
                        referenceNode);
                if (createdEntity.isPresent()) {
                    OWLEntity entity = createdEntity.get();
                    Set<OWLAxiom> axioms = createOWLAxioms(entity, entityLabel, languageTag, referenceType,
                            referenceNode);
        ...
        //     /*
        //      * Get the literal value by resolving the given reference against the input
        //      * spreadsheet
        //      */
        ...
        //     } else if (referenceNode.hasEntityType()) {
        //         /*
        //          * Setup the entity name, label and language tag based on user's input. There
        //          * are validation process for the entity name and label to decide if an empty
        //          * value will throw a warning, an error or just simply ignore it.
        //          */
        ...
        //         /*
        //          * Create the OWL entity based on the input of its name, label and language tag.
        //          * The method createOWLEntity will check first if the name (or label) exists in
        //          * the ontology.
        //          */
        //         OWLEntityReferenceRendering entityRendering = null;
        //         Optional<OWLEntity> createdEntity = createOWLEntity(entityName, entityLabel, languageTag,
        ...
        //             "Unknown type '" + referenceType + "' for reference node: " + referenceNode);
        }
        throw new InternalRendererException("Unknown definition for reference node: " + referenceNode);
    }

    // private Optional<String> processResolvedValue(Optional<String> resolvedValue, SpreadsheetLocation location,
    ...
    // private Optional<String> processLiteral(Optional<String> literalValue, SpreadsheetLocation location,
    //         ReferenceDirectives directives) throws RendererException {
    ...
    // private Optional<String> processIdentifier(Optional<String> entityName, SpreadsheetLocation location,
    ...
    // private Optional<String> validateLabel(Optional<String> entityLabel, SpreadsheetLocation location,
    //         ReferenceNode referenceNode) throws RendererException {
    ...
    public Optional<OWLEntity> createOWLEntity(Optional<String> entityName, Optional<String> entityLabel,
            Optional<String> language, ReferenceNode referenceNode) throws RendererException {
    ...
    private OWLEntity createOWLEntityUsingEntityName(String entityName, ReferenceNode referenceNode,
            ReferenceDirectives directives) throws RendererException {
        OWLEntity entity = null;
        ...
        if (foundEntity.isPresent()) {
            if (isCompatible(foundEntity.get(), referenceType)) {
                ...
                case MM_ERROR_IF_OWL_ENTITY_EXISTS:
                    throw new RendererException(
                            "An entity with identifier '" + entityName + "' already exists in the ontology");
                case MM_RESOLVE_IF_OWL_ENTITY_EXISTS:
                    entity = foundEntity.get();
                }
            } else {
                String message = String.format(
                        "The entity '%s' was found as [%s] in the ontology but was mentioned as [%s] "
                                + "in the transformation rule",
                        entityName, foundEntity.get().getEntityType(), referenceType.getTypeName());
            ...
            case MM_ERROR_IF_OWL_ENTITY_DOES_NOT_EXIST:
                throw new RendererException("An entity with identifier '" + entityName
    ...
    private OWLEntity createOWLEntityUsingEntityLabel(String displayName, Optional<String> languageTag,
            ReferenceNode referenceNode, ReferenceDirectives directives) throws RendererException {
        OWLEntity entity = null;
        ...
        if (foundEntity.isPresent()) {
            if (isCompatible(foundEntity.get(), referenceType)) {
                ...
                case MM_ERROR_IF_OWL_ENTITY_EXISTS:
                    throw new RendererException(
                            "An entity with display name '" + displayName + "' already exists in the ontology");
                case MM_RESOLVE_IF_OWL_ENTITY_EXISTS:
                    entity = foundEntity.get();
                }
            } else {
                String message = String.format(
                        "The entity name '%s' was found as [%s] in the ontology but was mentioned as [%s] "
                                + "in the transformation rule",
                        displayName, foundEntity.get().getEntityType(), referenceType.getTypeName());
                throw new RendererException(message);
            }
        } else {
            int setting = directives.getActualIfOWLEntityDoesNotExistDirective();
            ...
            case MM_ERROR_IF_OWL_ENTITY_DOES_NOT_EXIST:
                throw new RendererException(
                        "An entity with display name '" + displayName + "' does not exist in the ontology.\n"
    ...
    private String encodeLocalName(String localName, ReferenceDirectives directives) {
        ...
        case MM_CAMELCASE_ENCODE:
            localName = NameUtil.toLowerCamel(localName);
            break;
        case MM_SNAKECASE_ENCODE:
            localName = NameUtil.toSnakeCase(localName);
            break;
        case MM_UUID_ENCODE:
            localName = NameUtil.toUUID();
            break;
        case MM_HASH_ENCODE:
            localName = NameUtil.toMD5(localName);
            break;
        }
        return localName;
    }
    ...
    private Optional<OWLEntity> getOWLEntityFromOntology(String displayName, Optional<String> languageTag)
    ...
    public Set<OWLAxiom> processTypesClause(OWLNamedIndividual individual, List<TypeNode> typeNodes)
            throws RendererException {
        ...
        for (TypeNode typeNode : typeNodes) {
            ...
            if (obj instanceof OWLEntity) {
                ...
                if (entity.isOWLClass()) {
                    OWLClassAssertionAxiom axiom = objectFactory.createOWLClassAssertionAxiom(entity.asOWLClass(),
                            individual);
                    axioms.add(axiom);
                } else {
                    IRI identifier = individual.getIRI();
                    throw new RendererException(
                            "Expecting OWL class for individual " + identifier + ", got " + entity.getClass());
                }
                ...
                IRI identifier = individual.getIRI();
                throw new RendererException(
                        "Unsupported type '" + typeNode.getNodeName() + "' for individual " + identifier);
            }
        }
        return axioms;
    }
    ...
    /*
     * This method will return either a short name or an IRI string, depending on
     * the directive in the reference node.
     */
    ...
    private void addOWLEntityAxiom(OWLEntity entity, ReferenceType referenceType, ReferenceNode referenceNode,
            Set<OWLAxiom> axioms) throws RendererException {
            ...
            if (typeRendering.isPresent()) {
                ...
                if (typeObject instanceof OWLEntity) {
                    ...
                    if (referenceType.isOWLClass() && typeEntity instanceof OWLClass) {
                        OWLAxiom axiom = objectFactory.createOWLSubClassOfAxiom(typeEntity.asOWLClass(),
                                entity.asOWLClass());
                        axioms.add(axiom);
                    } else if (referenceType.isOWLNamedIndividual() && typeEntity instanceof OWLClass) {
                        OWLAxiom axiom = objectFactory.createOWLClassAssertionAxiom(typeEntity.asOWLClass(),
                                entity.asOWLNamedIndividual());
                        axioms.add(axiom);
                    } else if (referenceType.isOWLObjectProperty() && typeEntity instanceof OWLObjectProperty) {
                        OWLAxiom axiom = objectFactory.createOWLSubObjectPropertyOfAxiom(
                                typeEntity.asOWLObjectProperty(), entity.asOWLObjectProperty());
                        axioms.add(axiom);
                    } else if (referenceType.isOWLDataProperty() && typeEntity instanceof OWLDataProperty) {
                        OWLAxiom axiom = objectFactory.createOWLSubDataPropertyOfAxiom(typeEntity.asOWLDataProperty(),
    ...
    private void addOWLAnnotationAxiom(OWLEntity entity, Optional<String> label, Optional<String> language,
            Set<OWLAxiom> axioms) {
        if (label.isPresent()) {
        ...
        if (value.isPresent()) {
            ...
            if (referenceNode.hasLiteralValueEncoding()) {
                if (referenceNode.hasExplicitlySpecifiedLiteralValueEncoding()) {
                    finalValue = generateReferenceValue(inputValue, referenceNode.getLiteralValueEncodingNode(),
                            referenceNode);
    ...
    private Optional<String> getEntityName(Optional<String> value, ReferenceNode referenceNode)
            throws RendererException {
        ...
        if (value.isPresent()) {
            ...
            if (referenceNode.hasRDFIDValueEncoding()) {
                if (referenceNode.hasExplicitlySpecifiedRDFIDValueEncoding()) {
                    finalName = generateReferenceValue(inputValue, referenceNode.getRDFIDValueEncodingNode(),
                            referenceNode);
                } else if (referenceNode.hasValueExtractionFunctionNode()) {
    ...
    private Optional<String> getEntityLabel(Optional<String> value, ReferenceNode referenceNode)
            throws RendererException {
        ...
        if (value.isPresent()) {
            ...
            if (referenceNode.hasRDFSLabelValueEncoding()) {
                if (referenceNode.hasExplicitlySpecifiedRDFSLabelValueEncoding()) {
                    finalLabel = generateReferenceValue(inputValue, referenceNode.getRDFSLabelValueEncodingNode(),
                            referenceNode);
                } else if (referenceNode.hasValueExtractionFunctionNode()) {
    ...
    private String generateReferenceValue(@Nonnull String value, ValueEncodingDirectiveNode valueEncodingDirectiveNode,
        ...
        if (valueEncodingDirectiveNode != null) {
            if (valueEncodingDirectiveNode.hasValueSpecificationNode()) {
                return generateReferenceValue(value, valueEncodingDirectiveNode.getValueSpecificationNode(),
                        referenceNode);
            }
        }
        return value;
    ...
    private String generateReferenceValue(@Nonnull String value, ValueSpecificationNode valueSpecificationNode,
        ...
        for (ValueSpecificationItemNode specificationItemNode : valueSpecificationNode
            ...
            if (specificationItemNode.hasStringLiteral()) {
                ...
                ValueExtractionFunctionNode valueExtractionFunction = specificationItemNode
    ...
    private String generateReferenceValue(@Nonnull String value,
    ...
    private OWLLiteral createOWLLiteral(String value, Optional<String> languageTag, ReferenceType referenceType)
            throws RendererException {
