...
public class TextRenderer extends ReferenceRendererConfiguration implements Renderer, ReferenceRenderer,
        DeclarationRenderer, EntityRenderer, LiteralRenderer, ClassExpressionRenderer, MappingMasterParserConstants {
    ...
    public Optional<? extends TextReferenceRendering> renderReference(ReferenceNode referenceNode)
        ...
        //         if (isCommented)
        //             rendering.addComment(createComment(literal, referenceNode));
        //         return Optional.of(rendering);
        ...
        //                         "Empty rdfs:label for " + referenceNode + " at location " + location);
        //             }
        //         }
        //         TextReferenceRendering rendering = createTextReferenceRendering(entityName, entityLabel, referenceNode);
        //         if (isCommented)
        //             rendering.addComment(createComment(entityLabel, referenceNode));
        //         return Optional.of(rendering);
        ...
        //         if (isCommented)
        //             rendering.addComment(createComment(iri, referenceNode));
        //         return Optional.of(rendering);
    ...
    private TextReferenceRendering createTextReferenceRendering(String entityName, String entityLabel,
    ...
    private String processOWLLiteralReferenceValue(Optional<String> resolvedValue, ReferenceNode referenceNode) throws RendererException {
        String literal = "";
        if (resolvedValue.isPresent()) {
            ...
            if (referenceNode.hasLiteralValueEncoding()) {
                if (referenceNode.hasExplicitlySpecifiedLiteralValueEncoding()) {
                    ...
                    ValueExtractionFunctionNode valueExtractionFunctionNode = referenceNode
    ...
    private String generateReferenceValue(String resolvedValue, ValueSpecificationNode valueSpecifications,
        ...
        for (ValueSpecificationItemNode specificationItem : valueSpecifications.getValueSpecificationItemNodes()) {
            if (specificationItem.hasStringLiteral()) {
                ...
                ValueExtractionFunctionNode valueExtractionFunction = specificationItem
                        .getValueExtractionFunctionNode();
    ...
    private String getReferenceRDFID(Optional<String> resolvedValue, ReferenceNode referenceNode)
            throws RendererException {
        String id = "";
        if (resolvedValue.isPresent()) {
            String value = resolvedValue.get();
    ...
    private String getReferenceRDFSLabel(Optional<String> resolvedValue, ReferenceNode referenceNode)
            throws RendererException {
        String label = "";
        if (resolvedValue.isPresent()) {
            String value = resolvedValue.get();
    ...
    public Optional<? extends TextRendering> renderOWLClassDeclaration(OWLClassDeclarationNode classDeclarationNode)
        ...
        if (classDeclarationNode.hasOWLEquivalentClassesNode()) {
            for (OWLEquivalentClassesNode equivalentToNode : classDeclarationNode.getOWLEquivalentClassesNodes()) {
                textRenderingResult = renderOWLEquivalentClasses(classDeclarationNode.getOWLClassNode(),
                        equivalentToNode);
                if (textRenderingResult.isPresent()) {
    ...
    public Optional<? extends TextRendering> renderOWLRestriction(OWLRestrictionNode restrictionNode)
        ...
        if (propertyRenderingResult.isPresent() && restrictionRenderingResult.isPresent()) {
            ...
            String textRepresentation = "(" + propertyRendering.getRendering() + " "
                    + restrictionRendering.getRendering() + ")";
    ...
    @Override
    public Optional<? extends TextRendering> renderOWLPropertyAssertion(OWLPropertyAssertionNode propertyAssertionNode)
            throws RendererException {
    ...
    @Override
    public Optional<? extends TextRendering> renderOWLClass(OWLClassNode classNode, boolean isDeclaration)
            throws RendererException {
    ...
    @Override
    public Optional<? extends TextRendering> renderOWLDataProperty(OWLPropertyNode propertyNode)
            throws RendererException {
        if (propertyNode.hasReferenceNode()) {
            return renderReference(propertyNode.getReferenceNode());
        } else if (propertyNode.hasNameNode()) {
    ...
    @Override
    public Optional<? extends TextLiteralRendering> renderOWLLiteral(OWLLiteralNode literalNode)
            throws RendererException {
    ...
    private Optional<? extends TextRendering> renderOWLSomeValuesFrom(OWLPropertyNode propertyNode,
        ...
        if (someValuesFromNode.hasOWLDataSomeValuesFromNode()) {
            valueRendering = renderOWLDataSomeValuesFrom(propertyNode,
    ...
    private Optional<? extends TextRendering> renderOWLHasValue(OWLPropertyNode propertyNode,
        ...
        if (hasValueNode.hasReferenceNode()) {
            ...
            Optional<? extends TextLiteralRendering> literalRendering = renderOWLLiteral(
                    hasValueNode.getOWLLiteralNode());
            if (literalRendering.isPresent()) {
                if (literalRendering.get().isQuoted()) {
                    valueRendering = Optional
                            .of(new TextLiteralRendering(quotes(literalRendering.get().getRendering())));
                } else {
